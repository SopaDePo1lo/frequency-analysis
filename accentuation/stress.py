...
def load():
    with open(file="accentuation/lemmas.dat", mode='rb') as f:
        lemmas = pickle.loads(f.read())
    with open(file="accentuation/wordforms.dat", mode='rb') as f:
...
def replace_upper(text):
    for i in range(len(not_accented)):
        text = text.replace(not_accented[i], accented[i])
    return text


def to_sound(text):
    return replace_upper(accentuate(text, wordforms, lemmas))
